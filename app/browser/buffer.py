...
from core.utils import touch, interactive, is_port_in_use, eval_in_emacs, message_to_emacs, set_emacs_var, translate_text, open_url_in_new_tab, read_emacs_var
...
class AppBuffer(BrowserBuffer):
    def __init__(self, buffer_id, url, config_dir, arguments, emacs_var_dict, module_path):
        ...
        self.history_list = []
        if read_emacs_var("eaf-browser-remember-history"):
    ...
    def update_progress(self, progress):
        ...
        if progress < 100:
            ...
            if self.dark_mode_is_enabled():
                if read_emacs_var("eaf-browser-dark-mode") == "follow":
    ...
    def after_page_load_hook(self):
        ...
        if read_emacs_var("eaf-browser-enable-adblocker"):
            self.load_adblocker()
    ...
    def try_start_aria2_daemon(self):
        ...
        if not is_port_in_use(6800):
            with open(os.devnull, "w") as null_file:
                ...
                aria2_args.append("--auto-file-renaming={}".format(str(read_emacs_var("eaf-browser-aria2-auto-file-renaming"))))
                aria2_args.append("-d {}".format(os.path.expanduser(read_emacs_var("eaf-browser-download-path"))))

                aria2_proxy_host = read_emacs_var("eaf-browser-aria2-proxy-host")
                aria2_proxy_port = read_emacs_var("eaf-browser-aria2-proxy-port")
    ...
    def record_close_page(self, url):
        ...
        if read_emacs_var("eaf-browser-remember-history") and self.arguments != "temp_html_file" and url != "about:blank":
    ...
    def toggle_adblocker(self):
        ...
        elif not read_emacs_var("eaf-browser-enable-adblocker"):
    ...
    def set_adblocker(self, url):
        if read_emacs_var("eaf-browser-enable-adblocker") and not self.page_closed:
            self.load_adblocker()
    ...
    def init_pw_autofill(self):
        if read_emacs_var("eaf-browser-enable-autofill"):
    ...
    def toggle_password_autofill(self):
        ...
        if not read_emacs_var("eaf-browser-enable-autofill"):
    ...
    def record_history(self, new_title):
        ...
        if read_emacs_var("eaf-browser-remember-history") and self.buffer_widget.filter_title(new_title) != "" and \
    ...
    def new_blank_page(self):
        ''' Open new blank page.'''
        eval_in_emacs('eaf-open', [read_emacs_var("eaf-browser-blank-page-url"), "browser", "", 't'])
    ...
    def _import_chrome_history(self):
        dbpath = os.path.expanduser(read_emacs_var("eaf-browser-chrome-history-file"))
    ...
    def translate_page(self):
        ...
        translate_language = read_emacs_var("eaf-browser-translate-language")
