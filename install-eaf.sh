...
if [ "$(command -v pip3)" ]; then
    pip3 install --user pymupdf epc retrying pytaglib || { echo "Failed to install dependency with pip3."; exit 1;}
elif [ "$(command -v pip)" ]; then
    pip install --user pymupdf epc retrying pytaglib || { echo "Failed to install dependency with pip."; exit 1;}
else
